...
stop_uss_and_restore_profile() {
	...
	if [ 'LoadState=loaded' = "$(systemctl --property LoadState show "$SRV")" ]; then
		# prevent future
		systemctl mask "$SRV"
		# cancel pending
...
upgrade_to_latest_test_errata() {
	...
	while current="$(ucr get version/version)-$(ucr get version/patchlevel)" && [ "$current" != "$prev" ]; do
		if [ -x /root/activate-errata-test-scope.sh ]; then
...
add_extra_apt_scope() {
	...
	if [ "$(echo "$SCOPE" | cut -c1-5)" = "http:" ] || [ "$(echo "$SCOPE" | cut -c1-6)" = "https:" ]; then
		...
		echo "deb [trusted=yes] http://omar.knut.univention.de/build2/ ucs_$(ucr get version/version)-0-$SCOPE/all/" >/etc/apt/sources.list.d/99_extra_scope.list
		echo "deb [trusted=yes] http://omar.knut.univention.de/build2/ ucs_$(ucr get version/version)-0-$SCOPE/\$(ARCH)/" >>/etc/apt/sources.list.d/99_extra_scope.list
	fi
